...
上述 M-dominant shift-and-invert Krylov--Galerkin ROM 是仓库的早期实现记录；当前 \code{AlpsSweep} 已升级为后文所述的双边 Lanczos--Padé，并在 P1 线性化中显式包含 $\beta_v(\omega)$ 与激励右端的一阶导数。
投影后的 ROM
\begin{equation}
...
\subsection{当前 ALPS：双边 Lanczos--Padé}

当前 \code{AlpsSweep} 不再使用早期的单边 Krylov--Galerkin MVP。离线阶段先在
展开频率 $\omega_0$ 调用 \code{assembler.assemble(omega0)} 得到与 direct path 一致的
$\bm A_0$ 与 $\bm b_0$，再由
\code{PolynomialPortMomentBuilder::buildLosslessLinearization()} 构造文档 P1 近似
\begin{equation}
  \bm A(t)=\bm A_0+t\bm A_1,\qquad
  \bm b(t)=\bm b_0+t\bm b_1,\qquad
  t=\frac{k_0^2-k_{0,\mathrm{exp}}^2}{k_{0,\mathrm{exp}}^2}.
\end{equation}
其中 $\bm A_1$ 同时包含质量矩阵项和端口传播常数 $\beta_v$ 的一阶导数，
$\bm b_1$ 则保留端口激励与功率归一化的一阶变化。

为把仿射右端写成常输入状态空间，代码使用一个标量增广自由度：
\begin{equation}
 \bm L_0=\begin{bmatrix}\bm A_0&-\bm b_0\\0&1\end{bmatrix},\quad
 \bm L_1=\begin{bmatrix}\bm A_1&-\bm b_1\\0&0\end{bmatrix},\quad
 \bm G=-\bm L_0^{-1}\bm L_1,\quad
 \bm z_0=\begin{bmatrix}\bm A_0^{-1}\bm b_0\\1\end{bmatrix}.
\end{equation}
\code{LanczosPadeModel} 对 $\bm G$ 和 $\bm G^{\transpose}$ 构造左右 Krylov 基
$\bm V_q,\bm W_q$，并通过两遍再双正交化保持
$\bm W_q^{\transpose}\bm V_q\simeq\bm I$。小矩阵
\begin{equation}
  \bm T_q=\bm W_q^{\transpose}\bm G\bm V_q
\end{equation}
在数值误差范围内为三对角矩阵。S11 与 S21 分别使用对应端口观测向量建立标量
$[q-1/q]$ Padé 模型；在线阶段求解
\begin{equation}
  (\bm I-t\bm T_q)\bm y=\bm W_q^{\transpose}\bm z_0
\end{equation}
并做端口功率归一化。正常路径使用 Thomas 递推，代价为 $\mathcal O(q)$，而不是
逐频点求解 Galerkin 稠密 ROM。

PARDISO 生命周期由 \code{FactorizedSolveSession} 管理：每个展开点一次数值分解，
全部左右 Lanczos 算子作用只做回代。默认采用频带中心单展开点；可靠的自适应多点
仍需加入全阶残差探针后启用。双正交误差、三对角泄漏、最终耦合量和击穿状态写入
\code{diagnostics.json}。
