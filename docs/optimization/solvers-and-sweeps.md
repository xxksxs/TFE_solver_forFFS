...
**落地状态**：单点 Padé 风格 ALPS（PRIMA/Galerkin 路径）已经合入主干。CLI `--sweep alps` 启用，详见 `alps-sweep/plan.md`；BP filter 基准 101 点扫频加速 ~12×，与 direct 偏差 < 1e-10。下一步是多展开点 + 自适应。
...
|------|--------|------|------|
| 自适应频点加密 | 高 | M | 计划中 |
| AWE 单点 Padé / ALPS Galerkin | 高 | M | **已实现 (`--sweep alps`)** |
