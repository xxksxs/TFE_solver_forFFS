...
把 `Application::runApplication` 内 ~196–251 行的 direct loop 抽成 `sweep::DirectSweep`；`mor::AlpsSweep` 移到 `sweep::AlpsSweep` 并实现 `ISweepStrategy`。`Application::runApplication` 主体缩到 ~50–80 行。`bpfem/mor/AlpsSweep.hpp` 留薄壳别名 + `[[deprecated]]`。
...
src/mor/AlpsSweep.cpp                         删（git mv）
CMakeLists.txt                                src/sweep/*.cpp 加；src/mor/AlpsSweep.cpp 移除；
...
# direct 5 频点
.\build_mkl\Release\bp_fem_solver.exe --max-sweep-points 5 --basis-order 1 --no-write-all-fields --out results_step4_direct

# ALPS 101 频点 (TFE-5 multi-mode)
.\build_mkl\Release\bp_fem_solver.exe --max-sweep-points 101 --basis-order 1 --port-method tfe --tfe-modes-per-port 5 --sweep alps --alps-krylov-order 30 --no-write-all-fields --out results_step4_alps

# direct 单频点 (TFE-5)
.\build_mkl\Release\bp_fem_solver.exe --max-sweep-points 1 --basis-order 1 --port-method tfe --tfe-modes-per-port 5 --sweep direct --no-write-all-fields --out results_step4_single
```
...
- **Pardiso 跨频点缓存**：AlpsSweep 内部新建一个 `MklPardisoSolver` 因式分解 A_0（这是 `buildOffline` 的硬要求，不能用 `ctx.solver`，因为 ctx.solver 的 pattern 缓存是给主装配矩阵用的）。`SweepContext::solver` 给 DirectSweep 用，AlpsSweep 内部自管直接求解器。
